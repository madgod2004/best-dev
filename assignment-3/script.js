...
async function loadBooks() {
    ...
    try {
        ...
        const response = await fetch('https://assignment3.rohanhussain.com/api/books/27100245');
        if (!response.ok) {
...
async function handleSearch() {
    ...
    try {
        ...
        const response = await fetch(`https://assignment3.rohanhussain.com/api/books/27100245/search?query=${encodeURIComponent(query)}`);
        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
        }
        const payload = await response.json();                  
